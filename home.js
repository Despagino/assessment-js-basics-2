...
let greeUser = (username) => {
    return  `Welcome back, ${username}`
}

console.log(greeUser("Gino"))
...
let canWeDeliver = (zipCode) => {
    if (deliveryAreaZipCodes.includes(zipCode)) {
        ...
        return `Sorry, we cant delivery to that address.`
    }
}
...
let canWeDeliverTwo = zipcode => {
    deliveryAreaZipCodes.forEach(el => {
        if (el === zipcode) {
            return `Youre in our delivery zone`
        } else {
            return `Sorry we cant delivery to your address.`
        }
    })
}
